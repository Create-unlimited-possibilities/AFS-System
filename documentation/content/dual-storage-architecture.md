---
id: dual-storage-architecture
title: 双重存储架构
sidebar_label: 双重存储架构
slug: /dual-storage-architecture
---

# 双重存储架构说明

## 概述

本项目采用**双重存储架构**，结合MongoDB和文件系统的优势：

- **MongoDB**: 结构化数据存储、实时查询（mongoserver容器）
- **文件系统**: 批量数据读取、RAG缓存（server容器）

## 🏗️ 架构设计

### 容器职责

```
┌────────────────────────────────────────────┐
│         server容器（业务逻辑）            │
│  ┌───────────────────────────────────┐  │
│  │  文件系统卷（本地挂载）           │  │
│  │  ./server/storage:/app/storage    │  │
│  │  - userdata/                      │  │
│  │    - A_set/                       │  │
│  │    - B_sets/                      │  │
│  │    - C_sets/                      │  │
│  └───────────────────────────────────┘  │
└──────────────┬─────────────────────────┘
               │ 网络访问
               ↓
┌──────────────────────────────────────────┐
│      mongoserver容器（数据库服务）      │
│  ┌───────────────────────────────────┐  │
│  │  MongoDB数据卷（本地挂载）        │  │
│  │  ./mongoserver/mongodb_data:      │  │
│  │  /data/db                         │  │
│  └───────────────────────────────────┘  │
└──────────────────────────────────────────┘
```

## 📂 目录结构

### 项目目录

```
${PROJECT_ROOT}/
├── server/
│   ├── storage/                 # 文件系统记忆数据
│   │   └── userdata/
│   │       ├── {userId}/
│   │       │   ├── A_set/     # 自己回答的问题（70个）
│   │       │   ├── B_sets/    # 家人协助回答（70个）
│   │       │   └── C_sets/    # 朋友协助回答（64个）
│   │
│   └── src/
│       ├── services/
│       │   ├── storageService.js  # 统一存储服务
│       │   ├── fileStorage.js     # 文件系统操作
│       │   ├── memoryLoader.js    # 批量读取记忆
│       │   └── vectorIndexService.js  # RAG索引
│       └── routes/
│           ├── questions.js      # 问题API
│           └── answers.js        # 答案API
│
├── mongoserver/
│   ├── mongodb_data/            # MongoDB数据文件
│   │   ├── *.wt                # WiredTiger数据
│   │   ├── collection-*.wt      # 集合数据
│   │   └── journal/            # 日志文件
│   └── init/                   # 初始化脚本
│
└── docker-compose.yml
```

## 🔄 数据写入流程

### 写入双重存储

```
用户填写答案
    ↓
POST /api/questions/answer
    ↓
server容器接收
    ↓
┌────────────────────────────────────────┐
│  StorageService.saveAnswer()          │
└────────────┬─────────────────────────┘
             │
       ┌──────┴──────┐
       ↓             ↓
┌──────────┐  ┌──────────────┐
│ MongoDB  │  │  文件系统      │
│ (同步）    │  │  (异步）        │
└──────────┘  └──────────────┘
       ↓             ↓
mongoserver/   server/
mongodb_data/   storage/
```

### 代码实现

```javascript
// server/src/services/storageService.js

export default class StorageService {
  async saveAnswer(answerData) {
    // 步骤1：写入MongoDB（同步）
    const dbAnswer = await Answer.findOneAndUpdate(
      { userId, targetUserId, questionId },
      { question, answer, questionLayer: layer },
      { upsert: true, new: true }
    );

    // 步骤2：异步写入文件系统
    this.syncToFileSystem(dbAnswer.toObject()).catch(err => {
      console.error('[StorageService] 文件同步失败:', err);
    });

    return { success: true, answer: dbAnswer };
  }
}
```

## 📖 数据读取流程

### 场景1：前端查看答案（MongoDB）

```
前端请求
    ↓
GET /api/answers?targetUserId={id}
    ↓
server容器
    ↓
┌────────────────────────────────────────┐
│  MongoDB查询（最新数据）               │
└────────────────────────────────────────┘
             ↓
          网络访问
             ↓
       mongoserver容器
             ↓
         MongoDB数据库
             ↓
         返回给前端
```

### 代码实现

```javascript
// server/src/routes/answers.js

router.get('/', async (req, res) => {
  const { targetUserId } = req.query;

  // 直接从MongoDB读取（最新数据）
  const answers = await Answer.find({ targetUserId })
    .sort({ createdAt: -1 })
    .lean();

  res.json({ success: true, answers });
});
```

### 场景2：生成角色卡（文件系统）

```
前端请求
    ↓
POST /api/chatbeta/rolecard/generate
    ↓
server容器
    ↓
┌────────────────────────────────────────┐
│  FileStorage.loadUserMemories()       │
│  - 批量读取JSON文件                  │
└────────────┬─────────────────────────┘
             ↓
      本地文件读取
             ↓
   server/storage/userdata/
   {userId}/A_set/self/emotional/
             ↓
        批量加载
             ↓
       返回数据结构
```

### 代码实现

```javascript
// server/src/services/memoryLoader.js

export default class MemoryLoader {
  async loadUserMemories(userId) {
    const memories = { A_set: [], B_sets: [], C_sets: [] };
    const userPath = path.join(this.basePath, String(userId));

    // 批量加载A_set
    const A_setPath = path.join(userPath, 'A_set', 'self');
    await this.loadMemoriesFromFolder(A_setPath, memories.A_set, 'basic');
    await this.loadMemoriesFromFolder(A_setPath, memories.A_set, 'emotional');

    // 批量加载B_sets
    const B_setsPath = path.join(userPath, 'B_sets');
    const B_folders = await fs.readdir(B_setsPath);
    for (const folder of B_folders) {
      await this.loadMemoriesFromFolder(folderPath, memories.B_sets, 'basic');
      await this.loadMemoriesFromFolder(folderPath, memories.B_sets, 'emotional');
    }

    return memories;
  }
}
```

### 场景3：RAG检索（ChromaDB向量索引）

```
用户发送消息
    ↓
POST /api/chatbeta/chat/send
    ↓
server容器
    ↓
┌────────────────────────────────────────┐
│  VectorIndexService.search()            │
└────────────┬─────────────────────────┘
             ↓
       ChromaDB查询
             ↓
    server/storage/chroma_db/
    user_{userId} 集合
             ↓
       返回相关记忆
```

### 代码实现

```javascript
// server/src/services/vectorIndexService.js

export default class VectorIndexService {
  async search(userId, query, topK = 5) {
    const collection = await this.chroma.getCollection({
      name: `user_${userId}`
    });

    const results = await collection.query({
      queryTexts: [query],
      nResults: topK
    });

    return results;
  }
}
```

## 🎯 数据使用策略

| 场景 | 数据来源 | 访问方式 | 原因 |
|------|---------|---------|------|
| **用户填写答案** | MongoDB + 文件系统 | 网络访问 + 本地写入 | 确保数据持久化 |
| **前端查看答案** | MongoDB | 网络访问 | 获取最新数据 |
| **生成角色卡** | 文件系统 | 本地读取 | 批量读取，高性能 |
| **RAG检索** | ChromaDB | 本地读取 | 快速语义检索 |
| **降级读取** | MongoDB | 网络访问 | 文件系统为空时使用 |

## 🔑 核心原则

### 1. 数据归属
- **MongoDB数据** → `mongoserver/mongodb_data/`（mongoserver容器负责）
- **文件系统数据** → `server/storage/userdata/`（server容器负责）
- 各容器管理自己的数据

### 2. 访问方式
- **MongoDB**: 通过网络访问 (`mongodb://mongoserver:27017`)
- **文件系统**: 通过绑定挂载访问 (本地卷)

### 3. 写入策略
- **MongoDB**: 同步写入，确保数据不丢失
- **文件系统**: 异步写入，不影响响应速度

### 4. 读取策略
- **实时查询**: 从MongoDB读取（最新数据）
- **批量处理**: 从文件系统读取（高性能）

## 📊 数据统计

### 当前后端数据
- 用户数：13
- 答案记录：270条
- 记忆文件：270个JSON

### 存储空间
- MongoDB数据：约XX MB (`mongoserver/mongodb_data/`)
- 文件系统数据：约XX MB (`server/storage/userdata/`)

## 💾 数据备份

### 备份MongoDB

```bash
# mongodump备份
docker exec afs-system-mongoserver-1 mongodump --db afs_db --out /tmp/backup
docker cp afs-system-mongoserver-1:/tmp/backup ./backup/

# 或直接复制文件
copy mongoserver\mongodb_data\ backup\
```

### 备份文件系统

```bash
# 复制整个storage目录
copy server\storage\ backup\
```

## ⚠️ 注意事项

### 1. Git协作
- 数据文件已添加到 `.gitignore`
- 不要提交敏感数据到Git
- 其他人克隆后会创建空目录

### 2. 容器重启
- 数据持久化在本地目录
- 容器重启不会丢失数据

### 3. 跨容器访问
- MongoDB：通过网络端口访问
- 文件系统：每个容器有自己的卷

---

**最后更新**: 2026-02-03  
**维护者**: 项目团队